...
[__UPDATE:__ Yes we need to send the Register ID + Register Value as I2C Sub Address, followed by an I2C Read. See the Appendix](https://github.com/lupyuen/rust-i2c-nuttx#fix-i2c-write)
...
[__UPDATE:__ Nope it's not writing correctly. We need to send the Register ID + Register Value as I2C Sub Address, followed by an I2C Read. See the Appendix](https://github.com/lupyuen/rust-i2c-nuttx#fix-i2c-write)
...
[(__bme280_set_interval__ and __bme280_set_standby__ were cloned from the NuttX BMP280 Driver)](https://github.com/lupyuen/incubator-nuttx/blob/bme280/drivers/sensors/bmp280.c#L330-L565)
...
NuttX's BMP280 Driver implements a [__Barometer Sensor__](https://github.com/lupyuen/incubator-nuttx/blob/bme280/include/nuttx/sensors/sensor.h#L326-L331) that provides Pressure and Temperature values.
...
-   [__Barometer Sensor: /dev/sensor/baro0__](https://github.com/lupyuen/incubator-nuttx/blob/bme280/include/nuttx/sensors/sensor.h#L326-L331)

    (For Pressure and Temperature)

-   [__Humidity Sensor: /dev/sensor/humi0__](https://github.com/lupyuen/incubator-nuttx/blob/bme280/include/nuttx/sensors/sensor.h#L345-L349)
...
The function above is called at startup by __bl602_bringup__: [bl602_bringup.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/boards/risc-v/bl602/bl602evb/src/bl602_bringup.c#L623-L640)    
...
__For ESP32:__ Edit the function [esp32_bringup](https://github.com/lupyuen/incubator-nuttx/blob/bme280/boards/xtensa/esp32/esp32-devkitc/src/esp32_bringup.c#L134-L497) in [esp32_bringup.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/boards/xtensa/esp32/esp32-devkitc/src/esp32_bringup.c#L134-L497) so that it calls [bme280_register](https://github.com/lupyuen/bme280-nuttx/blob/main/driver.c#L668-L773)
...
Nope this won't compile with NuttX, because it assumes that every Sensor Driver __lives in its own file__. [(Like bmp280.c)](https://github.com/lupyuen/incubator-nuttx/blob/bme280/drivers/sensors/bmp280.c)
...
And we create a __Symbolic Link__ at [__drivers/sensors/bme280.c__](https://github.com/lupyuen/incubator-nuttx/blob/bme280/drivers/sensors/bme280.c)...
...
Which is Sym-Linked at [__include/nuttx/sensors/bme280.h__](https://github.com/lupyuen/incubator-nuttx/blob/bme280/include/nuttx/sensors/bme280.h)
...
Let's take a quick look inside the Sensor Test App, so that we can code our own Sensor Apps: [sensortest.c](https://github.com/lupyuen/incubator-nuttx-apps/blob/bme280/testing/sensortest/sensortest.c#L233-L401)
...
int main(int argc, FAR char *argv[]) {
  ...
  //  len is Sensor Data size: sizeof(struct sensor_event_baro)
  FAR char *buffer = calloc(1, len);
```

The app begins by allocating a buffer for the Sensor Data Struct: __sensor_event_baro__ (Barometer) or __sensor_event_humi__ (Humidity).

Next we open the Sensor Device: __/dev/sensor/baro0__ (Barometer) or __/dev/sensor/humi0__ (Humidity)...

```c
  //  Open the Sensor Device.
  //  devname looks like "/dev/sensor/baro0"
...
We enable the sensor by switching to [__Normal Power Mode__](https://lupyuen.github.io/articles/bme280#power-management)...
...
Finally we disable the sensor and switch to [__Low Power Mode__](https://lupyuen.github.io/articles/bme280#power-management)...
...
[__g_sensor_info__](https://github.com/lupyuen/incubator-nuttx-apps/blob/bme280/testing/sensortest/sensortest.c#L77-L110) is a Lookup Table that defines the __Print Function__ for every Sensor Type.

(Remember that the Sensor Test App needs to handle all Sensor Types)

Here's the Print Function for our __Barometer Sensor__: [sensortest.c](https://github.com/lupyuen/incubator-nuttx-apps/blob/bme280/testing/sensortest/sensortest.c#L146-L151)
...
And here's the Print Function for our __Humidity Sensor__: [sensortest.c](https://github.com/lupyuen/incubator-nuttx-apps/blob/bme280/testing/sensortest/sensortest.c#L139-L144)
...
static void print_valf(const char *buffer, const char *name) {
  ...
  struct sensor_event_prox *event = 
    (struct sensor_event_prox *) buffer;
...
_But we're printing Humidity, not Proximity... Why cast to sensor_event_prox?_

Yep we should cast to __sensor_event_humi__ instead...

```c
//  Cast buffer as Humidity Sensor Data
struct sensor_event_humi *event = 
  (struct sensor_event_humi *) buffer;
```
...
The Print Functions are defined in the Lookup Table __g_sensor_info__: [sensortest.c](https://github.com/lupyuen/incubator-nuttx-apps/blob/bme280/testing/sensortest/sensortest.c#L77-L110)
...
But first we need to wrap up the [__NuttX I2C ioctl Interface__](https://github.com/lupyuen/incubator-nuttx-apps/tree/bme280/system/i2c) as a [__Rust Embedded HAL__](https://docs.rs/embedded-hal/0.2.6/embedded_hal/blocking/i2c/index.html).

We'll cover this in the next article. Stay tuned!
...
1.  Why does this I2C error appear after reading the BME280 sensor?
    ...
    Perhaps because BME280 is in Low Power Mode?
    ...
    NuttX's BMP280 Driver writes a different value to Register F4 and doesn't hit the error...
...
We see this in the BL602 NuttX I2C Driver: [bl602_i2c.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/arch/risc-v/src/bl602/bl602_i2c.c#L710-L742)
...
Below is the code from the BL602 I2C Driver that configures the I2C Sub Address for each I2C Transfer: [bl602_i2c.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/arch/risc-v/src/bl602/bl602_i2c.c#L275-L319)
...
For sending I2C data: [bl602_i2c.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/arch/risc-v/src/bl602/bl602_i2c.c#L194-L197)
...
For receiving I2C data: [bl602_i2c.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/arch/risc-v/src/bl602/bl602_i2c.c#L207-L216)
...
For I2C transfers: [bl602_i2c.c](https://github.com/lupyuen/incubator-nuttx/blob/bme280/arch/risc-v/src/bl602/bl602_i2c.c#L740-L742)
...
1.  Enable __BME280 Driver__...
    ...
    The BME280 Driver will appear in NuttX at __/dev/sensor/baro0__ and __/dev/sensor/humi0__
