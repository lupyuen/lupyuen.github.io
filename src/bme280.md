...
This returns a Zephyr [__sensor_value__](https://docs.zephyrproject.org/latest/reference/peripherals/sensor.html#c.sensor_value) struct that represents the Pressure as a [__Fixed-Point Number__](https://en.wikipedia.org/wiki/Fixed-point_arithmetic).

We convert the Pressure from __Fixed Point to Float__...

```c
  //  Convert pressure to float, scale up by 10 to get millibars
...
We do the same to get the __Temperature__ from the fetched sample...
