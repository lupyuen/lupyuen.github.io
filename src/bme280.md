...
static int bme280_fetch(
  ...
  FAR struct sensor_event_baro *baro_data,  //  Returns pressure and timestamp
  FAR struct sensor_event_humi *humi_data   //  Returns humidity
) {
...
```c
  ...
  int ret = bme280_sample_fetch(priv, SENSOR_CHAN_ALL);
  if (ret < 0) { return ret; }
```
