...
1.  _How will we boot Apache NuttX RTOS on Ox64?_
    ...
    "Pretending to be Linux" has worked remarkably well for booting NuttX on PinePhone, PinePhone Pro, NanoPi M4 SBC and Star64 RISC-V SBC. We'll do it again for Ox64!

    TODO: Linux Image Header

    TODO: To speed up the porting, we might boot NuttX over TFTP.

1.  _How will we print to the Serial Console?_

    We'll borrow the NuttX UART Driver for BL602 (which will probably work with BL808)...

    TODO

    (Linux Device Tree has helpful pointers)

    Or we'll call OpenSBI Debug Console...

    TODO

1.  _Will we boot NuttX on both 32-bit and 64-bit RISC-V Cores?_

    Initially we'll boot NuttX on the 64-bit D0 Multimedia Core. NuttX will talk to the existing OpenBouffalo Firmware on the 32-bit M0 Wireless Core.

    TODOL (Linux Device Tree tells us how)

    Later we might boot NuttX on the 32-bit M0 Wireless Core.

    TODO: OpenAMP

1.  _OpenSBI will boot NuttX on the 32-bit RISC-V Core right?_

    Sadly nope. The 32-bit M0 Wireless Core isn't controlled by OpenSBI, according to the OpenSBI...

    TODO

    So booting 32-bit NuttX with OpenSBI HSM simply won't work.
...
1.  _Are you sorry that NuttX Fans had to put up with this entire article?_
    ...
    Yes I'm truly sorry! But that's exactly how we'll run NuttX on Ox64: Flash OpenSBI and U-Boot Bootloader to Ox64, so that it will load a NuttX Kernel Image. (That pretends to be Linux)

    Thankfully it's a only a One-Time Thing. To upgrade the NuttX Kernel, we only need to update the microSD Card. (No more flashing!)
