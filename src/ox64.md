...
[__Bouffalo Lab BL808 SoC__](https://github.com/bouffalolab/bl_docs/blob/main/BL808_RM/en/BL808_RM_en_1.3.pdf) is a curiously complex creature with 3 (Asymmetric) RISC-V Cores (pic below)...
...
Technically Ox64 boots __64-bit RISC-V Linux__ (via microSD), so Ox64 feels like a __RISC-V SBC__...

- [__Linux Image + OpenSBI + U-Boot Bootloader__ for BL808](https://github.com/openbouffalo/buildroot_bouffalo)
...
1.  Check that our __USB Serial Adapter__ is connected to the __Flashing UART__. (Pic above)
...
1.  We download the __Ox64 Binaries__...
...
1.  Set the Firmware for __M0 Wireless Core__...
    ...
    |              |          |
    |:-------------|:---------|
    | __M0 Group__ | __`group0`__ |
    | __Image Addr__ | __`0x58000000`__ |
    | &nbsp;
    
    Select the downloaded __m0_lowload_bl808_m0.bin__

1.  Set the Firmware for __D0 Multimedia Core__...

    |              |          |
    |:-------------|:---------|
    | __D0 Group__ | __`group0`__ |
    | __Image Addr__ | __`0x58100000`__ |
    | &nbsp;
    
    Select the downloaded __d0_lowload_bl808_d0.bin__
...
1.  Click "__Create & Download__" and for it to complete
...
1.  Set the __Single Download Options__...
    ...
    Select the downloaded __bl808-firmware.bin__
...
1.  Connect our USB Serial Adapter to the __Ox64 Serial Console__ (pic above)...
...
1.  Flash the __Uncompressed Image__ to our microSD card.
    ...
    Use [__Balena Etcher__](https://github.com/balena-io/etcher), [__GNOME Disks__](https://wiki.gnome.org/Apps/Disks) or [__`dd`__](https://gist.github.com/lupyuen/aae995d942d5ec3ffa6629667bcc3ae6).

1.  Insert the microSD Card into Ox64. (Pic above)

    ![Flashing UART](https://lupyuen.github.io/images/ox64-pinout2.jpg)

1.  Connect our __USB Serial Adapter__ to the __Ox64 Flashing UART__ (pic above)...
...
1.  Connect our __USB Serial Adapter__ to the __Ox64 Serial Console__ (pic above)...
...
1.  We should see [__d0_lowload__](https://github.com/openbouffalo/OBLFR/tree/master/apps/d0_lowload) starting on D0 Wireless Core...
...
Yeah. According to [__@gamelaster__](https://x.com/gamelaster/status/1719073156281798755?s=20)...
...
1.  _How to boot Apache NuttX RTOS on Ox64?_
    ...
    "Pretending to be Linux" has worked remarkably well for booting NuttX on [__PinePhone__](https://lupyuen.github.io/articles/what), [__PinePhone Pro__](https://github.com/apache/nuttx/pull/10193), [__NanoPi M4 SBC__](https://github.com/apache/nuttx/pull/10645) and [__Star64 RISC-V SBC__](https://lupyuen.github.io/articles/release)...
    
    We'll do it again for Ox64!
...
1.  _OpenSBI will boot NuttX on the 32-bit RISC-V Core?_
    ...
    We'll have to flash 32-bit NuttX to Ox64 with [__BL DevCube__](https://lupyuen.codeberg.page/articles/ox64.html#flash-opensbi-and-u-boot-bootloader). (Ouch!)
...
Thus we simply __reuse the code__ from NuttX Star64!
...
(We'll reuse the __BL602 UART Driver__ for NuttX)
...
_What's this forwarding of Peripheral Interrupts? (Pic above_
...
- __SD Card Interrupt__ is IRQ 33
  ...
  (BL808 Reference Manual, Page 561)
...
_How do we forward the Peripheral Interrupts?_
...
- [__Forward UART2 Interrupts__](https://github.com/openbouffalo/OBLFR/blob/master/components/mailbox/src/oblfr_mailbox.c#L103-L111)

- [__Forward USB Interrupts__](https://github.com/openbouffalo/OBLFR/blob/master/components/mailbox/src/oblfr_mailbox.c#L111-L119)

- [__Forward Ethernet Interrupts__](https://github.com/openbouffalo/OBLFR/blob/master/components/mailbox/src/oblfr_mailbox.c#L119-L127)

_Are there other ways to boot Linux on Ox64?_

According to [__@madushan1000__](https://x.com/madushan1000/status/1719069431580524720?s=20)...
