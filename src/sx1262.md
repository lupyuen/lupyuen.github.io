...
1.  In the previous chapter we started a __Background Thread__ that waits for GPIO Interrupts and __adds an Event__ to our Event Queue

1.  In our NuttX App, the __Foreground Thread__ runs an __Event Loop__ that handles every Event in our Event Queue. And processes the received message.
...
_What Events will be added to our Event Queue?__
...
static void init_event_queue(void) {
  ...
  //  Init the Event Queue by calling NimBLE Porting Queue
