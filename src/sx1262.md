...
-   [drivers/ioexpander/gpio.c (line 544)](https://github.com/lupyuen/incubator-nuttx/blob/lorawan/drivers/ioexpander/gpio.c#L544-L547)
...
UPDATE: Unfortunately the thread never terminates, so any NuttX App that calls NimBLE Callouts won't terminate either. (Even when we call exit()). We need a flag that will tell the thread to terminate.
