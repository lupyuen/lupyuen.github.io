...
Note that we're using the older __"nuttx"__ branch of the "lora_sx1262" repo, which __doesn't use GPIO Interface and NimBLE Porting Layer__.
...
[(__SX126xIoInit__ is defined here)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L65-L77)

In our Test App we call __read_registers__ and __SX126xReadRegister__ to read a bunch of SX1262 Registers. (`0x00` to `0x0F`)

In our SX1262 Library, __SX126xReadRegister__ calls __SX126xReadRegisters__ and __sx126x_read_register__ to read each register: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L268-L281)
...
__sx126x_read_register__ reads a register by sending the Read Register Command to SX1262 over SPI: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L495-L504)
...
1.  Download the modified source code...
    ...
    Note that we're using the older __"sx1262"__ branch of the NuttX OS and NuttX Apps repos, which __don't use GPIO Interface and NimBLE Porting Layer__.
...
Our SX1262 Library opens the SPI Test Driver to __initialise the SPI Bus__ like so: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L663-L675)
...
[(Source)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L677-L703)

## Transfer SPI

To __transfer SPI Data__ to SX1262 via our SPI Test Driver, we do this: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L677-L703)
...
Note that __Write Buffer Offset is always 0__, because of [__SX126xSetPayload__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x.c#L144-L147) and [__SX126xWriteBuffer__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L283-L289).
...
__sx126x_hal_write__ calls __transfer_spi__ to transfer the Command Buffer and Data Buffer over SPI: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L544-L581)
...
They are the internal __1024-byte buffers for SPI Transfers__: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L533-L540)
...
Later as we inspect the code that __receives LoRa Messages__, we'll see that our SX1262 Library calls this function when a __Receive Done Event__ is triggered: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L514-L522)
...
__sx126x_hal_read__ calls __transfer_spi__ to transfer the Command Buffer over SPI: [sx126x-nuttx.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L583-L627)
...
__UPDATE:__ GPIO Interfaces have been implemented, this section shall be updated shortly
...
-   __BUSY__: Instead of reading this pin to check if SX1262 is busy, we __sleep 10 milliseconds__.
    ...
    [(See this)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L185-L200)

-   __NRESET__: To reset the SX1262 module, we __manually restart our NuttX gadget__.

    [(See this)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L149-L169)
...
1.  Initialise the GPIO Pins: [__SX126xIoInit__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L65-L77)

    [(Similar to BL602 IoT SDK)](https://lupyuen.github.io/articles/lorawan#appendix-bl602-spi-functions)

1.  Register GPIO Interrupt Handler for DIO1: [__SX126xIoIrqInit__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L79-L91)

    [(Similar to BL602 IoT SDK)](https://lupyuen.github.io/articles/lorawan#appendix-bl602-gpio-interrupts)

1.  Reset SX1262 via GPIO: [__SX126xReset__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L149-L169)
...
1.  Check SX1262 Busy State via GPIO: [__SX126xWaitOnBusy__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L185-L200)
...
1.  Get DIO1 Pin State: [__SX126xGetDio1PinState__](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L337-L344)
...
__UPDATE:__ NimBLE Porting Layer has been implemented, this section shall be updated shortly
...
In this section we explain the Platform-Independent __Radio Functions__, which are defined in [radio.c](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/radio.c)
...
[(__TimerInit__ is defined here)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L376-L395)
...
[(__TimerStart2__ is defined here)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L465-L490)
...
[(__TimerStop__ is defined here)](https://github.com/lupyuen/lora-sx1262/blob/lorawan/src/sx126x-nuttx.c#L397-L409)
