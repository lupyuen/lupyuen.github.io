...
We may flash BL706 with __OpenOCD and a JTAG or J-Link Adapter__.
...
We spotted some differences between the SDKs...
...
1.  __AliOS Functions__ for multitasking and device management are no longer in BL706 MCU SDK.

1.  __JTAG and J-Link__ are now supported for flashing BL706. (But not BL602)

1.  __LVGL (Graphics) and TensorFlow Lite (Machine Learning)__ are now bundled with BL706 MCU SDK

    [Sample Firmware for TensorFlow Lite](https://github.com/bouffalolab/bl_mcu_sdk/tree/master/examples/tensorflow)

1.  BL706 builds with __`cmake` and `make`__. (BL602 builds with `make` only)

    ![BL706 uses cmake](https://lupyuen.github.io/images/bl706-cmake.png)

1.  TODO: GCC Toolchain not bundled
...
1.  TODO: Porting from STM32CUBEMX
