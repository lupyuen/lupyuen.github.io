...
In the JavaScript generated by PureScript, we point the __JavaScript Imports__ to __compile.purescript.org__, so we don't need to deploy the Other JavaScript Modules: [run.sh](https://github.com/lupyuen/nuttx-purescript-parser/blob/main/run.sh)
...
<script type=module>
  ...
  // Allow other modules to access the PureScript Functions
