...
Actually there's a __Physical Switch__ that selects the Main CPU: __RISC-V OR Arm__.

Don't let yer pet hamster flip it... It will get super frustrating!

([__Sophgo__](https://sophon.ai/) / _sophon.ai_ refers to [__3 Body__](https://three-body-problem.fandom.com/wiki/Sophon)?)
...
With a tiny tweak, this will boot NuttX RTOS.
...
This produces the NuttX Image File: [__`Image-sg2000`__](https://github.com/lupyuen2/wip-nuttx/releases/download/sg2000-1/Image-sg2000). Which we copy to our __TFTP Server__...
...
We're eagerly awaiting the new 64-bit RISC-V SBCs based on the [__Sophgo SG2000 RISC-V SoC__](https://github.com/sophgo/sophgo-doc/releases). Meanwhile we've done our prep...
...
1.  Create an __SG2000 Emulator__ for easier testing
    ...
    [(Similar to __TinyEMU for Ox64__)](https://lupyuen.github.io/articles/tinyemu3)

1.  Someday we might run NuttX on the __SG2000 Co-Processor__

    (Also SG2002 with its upsized TPU / NPU)
...
Download the RISC-V Toolchain for __riscv64-unknown-elf__ (SiFive) or __riscv-none-elf__ (xPack)...
...
## Copy NuttX Image and Device Tree to TFTP Server.
## Device Tree from here: https://github.com/lupyuen2/wip-nuttx/releases/download/sg2000-1/cv181x_milkv_duos_sd.dtb
...
For now, we disable PLIC in NuttX...
...
Copy out the SG2000 __Device Tree Binary__: [cv181x_milkv_duos_sd.dtb](https://github.com/lupyuen/nuttx-sg2000/blob/main/cv181x_milkv_duos_sd.dtb)
...
Earlier we dumped the __Linux Device Tree__ for SG2000. We snoop inside to understand the Interrupt Controller...
